...
public class WndWeeklies extends Window {
	...
	public WndWeeklies(){
		...
		if (Rankings.INSTANCE.latestWeeklyReplay != null){
			IconButton replayInfo = new IconButton(Icons.get(Icons.CALENDAR)){
				@Override
				protected void onClick() {
					ShatteredPixelDungeon.scene().addToFront(new WndRanking(Rankings.INSTANCE.latestWeeklyReplay));
				}
		...
		ArrayList<Long> dates = new ArrayList<>(Rankings.INSTANCE.weeklyScoreHistory.keySet());
		Collections.reverse(dates);

		boolean first = Rankings.INSTANCE.latestWeekly != null;
		for (long l : dates) {
			if (first) top += 2;
			...
			if (first){
				IconButton latestInfo = new IconButton(Icons.INFO.get()){
					@Override
					protected void onClick() {
						ShatteredPixelDungeon.scene().addToFront(new WndRanking(Rankings.INSTANCE.latestWeekly));
					}
				};
			...
			score = PixelScene.renderTextBlock(num.format(Rankings.INSTANCE.weeklyScoreHistory.get(l)), 7);
			score.setPos(WIDTH - score.width(), top);
			content.add(score);
