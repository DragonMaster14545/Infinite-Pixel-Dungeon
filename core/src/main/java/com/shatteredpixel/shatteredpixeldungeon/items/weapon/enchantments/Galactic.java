...
public class Galactic extends Weapon.Enchantment {
    ...
    private void generateEnchantments() {
        ...
        for (int i = 0; i < limit; i++) {
            ...
            if ( enchantment instanceof Galactic) {
                limit += 3;
                // Just for display
                ((Galactic) enchantment).set();
            }
            additional.add( enchantment );
        }
    }

    public Galactic set(Weapon.Enchantment... enchants ) {
        additional.clear();
