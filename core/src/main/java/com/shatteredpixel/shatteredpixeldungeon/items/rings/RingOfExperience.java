...
public class RingOfExperience extends Ring {

	{
		icon = ItemSpriteSheet.Icons.RING_EXPERIENCE;
	}
	
	public String statsInfo() {
		if (isIdentified()){
			String info = Messages.get(this, "stats",
					new DecimalFormat("#.###").format(100f * (1.02f + soloVisualBonus()*0.02 - 1f)));
			if (isEquipped(Dungeon.hero) && soloBuffedBonus() != combinedBuffedBonus(Dungeon.hero)){
				info += "\n\n" + Messages.get(this, "combined_stats",
						Messages.decimalFormat("#.##", 100f * (1.02f + combinedBuffedBonus(Dungeon.hero)*0.02 - 1f)));
			}
			return info;
		} else {
	...
	public String upgradeStat1(long level){
		...
		return Messages.decimalFormat("#.##", 100f * (1.02f + level*0.02 - 1f)) + "%";
	}
