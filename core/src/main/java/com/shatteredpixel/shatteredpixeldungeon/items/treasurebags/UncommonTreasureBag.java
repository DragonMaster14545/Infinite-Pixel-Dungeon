...
public class UncommonTreasureBag extends TreasureBag {
    {
        image = ItemSpriteSheet.UNCOMMON_BAG;
    }

    @Override
    protected ArrayList<Item> items() {
        ArrayList<Item> items = new ArrayList<>();
        int amount = Random.Int(15, 40);
        ...
        for (Item item: items){
            ...
            if (item.isUpgradable() && Random.Float() < 0.33f) item.upgrade(1);
        }
        return items;
    }
