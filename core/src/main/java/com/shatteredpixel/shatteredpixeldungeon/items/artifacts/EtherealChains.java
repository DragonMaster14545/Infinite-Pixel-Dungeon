...
public class EtherealChains extends Artifact {
	...
	public class chainsRecharge extends ArtifactBuff {
		...
		public boolean act() {
			...
			if (charge < chargeTarget
				...
				float chargeGain = (1 / Math.max(1, 40f - (chargeTarget - charge) * 2f));
	...
	public static class WaveEternalChains extends EtherealChains {
		{
			charge = 25;
		...
		public class chainsRecharge extends ArtifactBuff {
			...
			@Override
			public boolean act() {
				if (!cursed && Regeneration.regenOn()) {
					float chargeGain = 1/4f;
					partialCharge += chargeGain;
				} else if (cursed && Random.Int(100) == 0) {
					Buff.prolong(target, Cripple.class, 10f);
				}
